...
namespace WordsToolkit.Scripts.Services.Ads.Networks
{
    ...
    public class IronsourceAdsHandler : AdsHandlerBase
    {
        ...
        private void SetConsentStatus()
        {
            ...
            Debug.Log($"IronSource consent status: {hasConsent}");

            // Set consent for GDPR
            IronSource.Agent.setConsent(hasConsent);

            // For CCPA compliance (optional)
            IronSource.Agent.setMetaData("do_not_sell", hasConsent ? "false" : "true");
            #elif IRONSOURCE
            // Default to no consent if UMP not available
            IronSource.Agent.setConsent(false);
            Debug.Log("UMP not available - setting IronSource consent to false");
            #endif
        }
